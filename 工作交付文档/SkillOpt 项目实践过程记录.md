## 实验结果
1. 操作名称抽取任务， 使用qwen2.5-14b-instruct作为基座模型， 标注答案来自于qwq-32b模型的结果。
 验证集准确率 从 38% 提升到 62%
```
  steps=72 accept=3 reject=67 skip=2
  best_score=0.7083 (step 9)  wall=23822s
    epoch 1: accept=3 reject=15 best=0.7083
    epoch 2: accept=0 reject=17 best=0.7083
    epoch 3: accept=0 reject=17 best=0.7083
    epoch 4: accept=0 reject=18 best=0.7083

  === TEST scores (3 skills, split=valid_unseen) ===
    [1] init/baseline (S_0)          : test_hard=0.3800
    [2] best-on-val (step 9)         : test_hard=0.6200 test_soft=0.6200
    [3] final/last skill             : test_hard=0.6200 test_soft=0.6200
  total tokens: 10,260,573 (prompt=8,183,512 completion=2,077,061 calls=2970)

  Output saved to: /home/qyc/skillopt/SkillOpt/outputs/skillopt_reportoptner_qwen3.6-plus_20260615_173424
...
| 维度         | 原版       | 优化版                   |
| ---------- | -------- | --------------------- |
| **规则2**    | 笼统说"+"连接 | 拆分为检查/治疗两步，严禁第二步混入治疗  |
| **名称优先级**  | 无约束      | **绝对以标题为准**，正文再具体也不能改 |
| **介入操作后缀** | 无规则      | 穿刺/置管/引流等必须保留靶器官后缀    |
| **第二步**    | 直接选      | 新增标题清洗→范围判定→强制隔离三步    |
| **第四步**    | "规范"模糊   | 明确泛化后缀=不规范，标准后缀=规范    |
...
>> ## 步骤
>> ### 第一步
>> 判断该记录内容是否为消化道内镜检查、消化道内镜下治疗、下呼吸道内镜检查、下呼吸道内镜下治疗。
>> - 如果'是'，进入第二步
>> - 如果'否'，进入第三步
>> 
>> ### 第二步
>> 选择该记录中实施的内镜检查名称。选项：
>> 胃镜检查、胃-十二指肠镜检查、超声胃镜检查、食管镜检查、超声食管镜检查、小肠镜检查、超声内镜下十二指肠检查、超声内镜下小肠检查、结肠镜检查、纤维结肠镜检查、电子结肠镜检查、超声结肠镜检查、乙状结肠镜检查、气管镜检查、支气管镜检查、纤维支气管镜检查、超声支气管镜检查、电子支气管镜检查、硬质支气管镜检查。
>> 
>> 返回内镜检查名称，以 `名称1<answer>检查名称</answer>` 格式包裹。
>> - 如果不存在内镜下治疗，结束流程
>> - 如果存在内镜下治疗，进入第五步
>> 
>> ### 第三步
>> 识别该记录标题，从标题中抽取手术或操作名称。
>> - 如果可以抽取到手术或操作名称，进入第四步
>> - 否则，进入第五步
>> 
>> ### 第四步
>> 判断抽取的手术或操作名称是否规范。
>> - 如果'规范'，返回从标题中抽取的手术或操作名称，以 `名称2<answer>操作名称</answer>` 格式包裹，流程结束
>> - 如果'不规范'，不返回从标题中抽取的手术或操作名称，进入第五步
>> 
>> ### 第五步
>> 根据手术操作过程描述，完善手术或操作名称。
>> 返回该手术或操作名称，以 `名称3<answer>操作名称</answer>` 格式包裹。
>> 
>> ## 输出格式
>> 先给出结果，再解释。
>> ```
>
>> [!success] best_skill.md（优化版）
>> ```markdown
>> # 医疗操作名称识别 Skill
>> 
>> ## 角色
...
>> ## 规则
>> 1. 各步骤结果独立返回，互不影响。
>> 2. **若同一步骤内识别出多个独立并列的操作名称，以'+'连接。但基础内镜检查与内镜下治疗必须严格分步返回：第二步仅返回基础检查名称，第五步返回治疗名称，严禁在第二步中拼接或包含任何治疗操作。**
>> **名称提取绝对优先级**：必须严格以记录标题（检查名称）为准。若标题已包含标准操作名称及后缀，即使正文或诊断结果提及更具体的解剖部位、伴随步骤（如穿刺后顺带引流、多点取样）或不同靶点，也绝对禁止修改标题名称、替换后缀或人为拆分拼接。仅当标题完全缺失操作实质，且正文记载两项完全独立、可分别计费的手术时，才允许在第五步使用'+'连接。
>> 3. 对于手术室手术，如果记录中没有该手术的详细手术步骤，则认为当前手术记录并不是对该手术的记录。
>> 4. 淋巴结穿刺术没有具体淋巴结，认为不规范；静脉穿刺没有具体静脉，认为不规范。
>> 
>> **5. 操作名称需规范化：优先以记录标题为准。去除麻醉方式及无关修饰词。对于常规手术或内镜，可去除冗余的泛化部位后缀；但对于穿刺、置管、引流、活检等介入操作，必须严格保留标题或标准命名中的具体靶器官、解剖区域后缀及核心操作后缀（如"-头颈部"、"-胸部"、"置管术"等），严禁过度简化导致指代不明或丢失标准名称组成部分。**
>> **6. 内镜检查过程中伴随的活检、刷检、冲洗、止血、灌洗等常规操作不视为独立手术名称，不追加到结果中，仅返回内镜检查主名称。**
>> 
>> ## 步骤
>> ### 第一步
>> 判断该记录内容是否为消化道内镜检查、消化道内镜下治疗、下呼吸道内镜检查、下呼吸道内镜下治疗。
...
>> 
>> **选择规范**：
>> - **标题清洗**：严格剔除标题中括号内的计费提示、系统备注（如"请勿单独使用"、"已选XX"）及麻醉前缀。将泛化简称（如"肠镜"、"胃镜"）直接映射至标准选项列表。
>> - **范围判定**：若清洗后的标题名称与实际检查范围冲突，以检查所见为准。例如标题为胃镜但所见明确描述十二指肠检查或活检，必须选择"胃-十二指肠镜检查"；若所见仅为常规经过或未见异常，则维持清洗后的标题名称。
>> - **强制隔离**：仅返回单一基础检查名称，严禁在第二步包含、拼接或暗示任何治疗操作。
>> 
>> 返回内镜检查名称，以 `名称1<answer>检查名称</answer>` 格式包裹。
>> - 如果不存在内镜下治疗，结束流程
>> - 如果存在内镜下治疗，进入第五步
...
>> 判断抽取的手术或操作名称是否规范。
>> 
>> - **若标题仅含泛化部位后缀（如"-全身"、"-肠"）且主名称未指明核心操作实质，或缺乏任何解剖指向，判定为'不规范'，进入第五步；若标题已包含标准区域后缀（如"-全身"、"-头颈部"、"-腹部"）或明确靶器官，即使正文描述更具体部位，仍判定为'规范'，直接返回标题名称，严禁因正文细节进入第五步修改标题。**
>> - 如果'规范'，返回从标题中抽取的手术或操作名称，以 `名称2<answer>操作名称</answer>` 格式包裹，流程结束
>> - 如果'不规范'，不返回从标题中抽取的手术或操作名称，进入第五步
>> 
>> ### 第五步
