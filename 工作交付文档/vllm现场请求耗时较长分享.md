...
- 隐藏维度: 5120
- 层数: 64
- 注意力头: 40 (GQA，KV heads=8)
**单token KV Cache** = 2(K+V) × 64层 × 5120 dim × 2字节 = **1.31 MB/token**

**峰值场景计算（20并发 × 长prompt 7400 tokens）**：

|场景|计算|KV Cache|
|:--|:--|:--|
|预填充阶段 (Prefill)|20 × 7400 × 1.31MB|**193 GB** ❌ 远超显存|
|实际场景（PagedAttention）|20 × 2048（典型窗口）|**53 GB**|
|P95场景（9并发）|9 × 4096|**48 GB**|
